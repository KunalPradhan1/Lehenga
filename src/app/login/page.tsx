            ...
            switch(response?.loginState){
                ...
                case LoginState.FAILURE:
                    if(response.errorCode === "invalidEmail" || response.errorCode === "invalidPassword"){
                        ...
                        setError("You need to reset you password!")
                    }
                    else{
            ...
            <form className="flex flex-col gap-8" onSubmit={handleSubmit}>
                ...
                {mode === MODE.REGISTER ? (
                    <div className="flex flex-col gap-2">
                        ...
                        <input type="text" name="username" placeholder="John" className="ring-2 ring-gray-300 rounded-md p-4" onChange={e=>setUsername(e.target.value)}></input>
                    </div>
                ) : null}
                {mode !== MODE.EMAIL_VERIFICATION ? (
                    <div className="flex flex-col gap-2">
                        <label className="text-sm text-gray-700">Email</label>
                        <input type="email" name="email" placeholder="John@gmail.com" className="ring-2 ring-gray-300 rounded-md p-4"onChange={e=>setEmail(e.target.value)}></input>
                    </div>
                ) : (
                    <div className="flex flex-col gap-2">
                        <label className="text-sm text-gray-700">Verification Code</label>
                        <input type="text" name="emailCode" placeholder="Code" className="ring-2 ring-gray-300 rounded-md p-4"onChange={e=>setEmailCode(e.target.value)}></input>
                    </div>
                )}
                {mode === MODE.LOGIN || mode === MODE.REGISTER ? (
                    <div className="flex flex-col gap-2">
                        <label className="text-sm text-gray-700">Password</label>
                        <input type="password" name="password" placeholder="Enter your password" className="ring-2 ring-gray-300 rounded-md p-4"onChange={e=>setPassword(e.target.value)}></input>
                    </div>
                ) : null}
                {mode === MODE.LOGIN && <div className="text-sm underline cursor-pointer" onClick={() => setMode(MODE.RESET_PASSWORD)}>Forgot Password</div>}
                <button className="bg-lehenga text-white p-2 rounded-md disabled:bg-pink-200 disabled:cursor-not-allowed" disabled={isLoading}>{isLoading ? "Loading..." : buttonTitle}</button>
